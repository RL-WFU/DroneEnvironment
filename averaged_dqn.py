...
class DQNAgent:
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            predicted = 0
            for k in range(0, self.num_of_prime):
                predicted += self.model_a[k].predict(next_state)
            predicted /= self.num_of_prime
            target = reward
            if not done:
    ...
    def update_target_params(self):
        self.model_a[0].set_weights(self.model.get_weights())
        for k in range(1, self.num_of_prime - 1):
            self.model_a[self.num_of_prime - k].set_weights(self.model_a[self.num_of_prime - k - 1].get_weights())
...
if __name__ == "__main__":
    ...
    for e in range(config.num_episodes):
        ...
        if e % average_over == 0:
            ...
            plt.xlabel('Episode')
            plt.savefig('adqn_reward.png')
            plt.clf()
            plt.plot(average_covered)
            plt.ylabel('Average Percent Covered')
            plt.xlabel('Episode')
            plt.savefig('adqn_average_coverage.png')
            plt.clf()
            plt.plot(episode_covered)
            plt.ylabel('Percent Covered')
            plt.xlabel('Episode')
            plt.savefig('adqn_coverage.png')
            plt.clf()
        if e == config.num_episodes - 6:
            env.plot_visited('adqn_drone_path1.jpg')
        if e == config.num_episodes - 5:
            env.plot_visited('adqn_drone_path2.jpg')
        if e == config.num_episodes - 4:
            env.plot_visited('adqn_drone_path3.jpg')
        if e == config.num_episodes - 3:
            env.plot_visited('adqn_drone_path4.jpg')
        if e == config.num_episodes - 2:
            env.plot_visited('adqn_drone_path5.jpg')
        if e == config.num_episodes - 1:
            env.plot_visited('adqn_drone_path6.jpg')

        if total_reward >= max_reward:
            max_reward = total_reward
            env.plot_visited('adqn_drone_max.jpg')
            agent.save('adqn_trained_weights.h5')
    ...
    plt.ylabel('Averaged Episode reward')
    plt.xlabel('Episode')
    plt.savefig('adqn_average_reward.png')
    plt.clf()
